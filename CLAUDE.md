...
- **비자명한 변경은 계획 우선**: 분석 → 편집 가능한 계획 파일(`plans/<slug>.md`) 작성 → 사용자 검토·승인
  → 실행 순서를 따른다. 이 절차는 `/plan` 커맨드로 캡슐화되어 있다. 사소한 변경은 예외.
...
현재 브랜치는 이 Simulink 시뮬레이션을 `Refactoring/` 아래의 순수 객체지향(OOP) MATLAB 구현으로 포팅한다.
2026-07-06 기준으로 이 포팅은 hover-to-cruise 천이 시나리오에 대한 **실행 가능한 end-to-end 폐루프(closed-loop)
