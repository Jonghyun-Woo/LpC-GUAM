...
%
% HJ toolbox 없이 MATLAB built-in만 사용하여
% LON BRT/FRT tube 위에 Full GUAM trajectory를 overlay.
%
% Required files:
%   data/GUAM_LON_BRT_UH%d_WH%d.mat
%
% Inputs:
%   R_or_trace:
%       - diagnostic result R, or
%       - R.trace directly
%
% opts fields:
...
% 각 UH schedule에서 local u = 0, local w = 0인 q-theta 단면을 계산한다.
    ...
    % axis_plot{1}이 trim-u만큼 이동된 absolute axis인 경우,
    % local u = 0에 해당하는 위치가 해당 schedule의 trim-u이다.
    uPlane = interp1( ...
        axes_base{1}, ...
        axis_plot{1}, ...
        0, ...
...
% Target q-theta 영역을 u = uPlane 평면의 사각형으로 표시한다.
...
function figName = make_overlay_filename(opts, short_labels, keep_dims)
    dimName = strjoin(short_labels(keep_dims), '_');

    figName = sprintf('%s_%s_%s_WH%d_%s', ...
        opts.filePrefix, ...
        upper(opts.tube), ...
        opts.coordMode, ...
        opts.wh_idx, ...
    ...
    figName = sanitize_filename(figName);
end
