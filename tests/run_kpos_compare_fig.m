% run_kpos_compare_fig - the mission with the tuned Q, at two outer
% position-loop gains.
%
% Left  : k_pos = 0.10 (shipped)
% Right : k_pos = 0.20
%
% The free-response test said 0.20 halves the standing altitude offset but
% takes the damping of the slow mode from 0.388 to 0.048, i.e. from decaying
% 93 % in one cycle to barely decaying at all. Whether that trade is worth it
% over a 45 s mission is not obvious from the free response, so this runs the
% actual mission both ways.
%
% Q is fixed at the tuned values for both panels.
clear; close all;
...
f = figure('Name','position-loop gain: 0.10 vs 0.20','Position',[40 40 1500 780]);
ax = gobjects(1,2);
fprintf('\nq1 = 0.08, q4 = 2.0\n\n');
fprintf('%7s | %9s %9s %9s %10s %8s\n', ...
        'k_pos','max|u-r|','e_pos','alt dev','alt swing','BRT>=0');
fprintf('%s\n', repmat('-',1,62));

for p = 1:2
    guam = LpC_GUAM(Config('trim_schedule', params));
    bc = guam.controller.baseline_controller;
    bc.LON.Ki = Ki;  bc.LON.Kx = Kx;  bc.k_pos = KP(p);
    guam.reset();
    ...
    fprintf('%7.2f | %9.2f %9.1f %9.1f %10.1f %8.1f\n', KP(p), eu, ep, ad, sw, viol);
    ...
    if p==1, tag='shipped'; else, tag='raised'; end
    title(ax(p), sprintf(['%s:  k_{pos} = %.2f   (q_1 = 0.08, q_4 = 2.0)\n' ...
          'speed %.2f ft/s | position %.1f ft | altitude %.1f ft | ' ...
          'alt swing %.1f ft'], tag, KP(p), eu, ep, ad, sw));
...
out = fullfile(root,'logger','kpos_compare');
