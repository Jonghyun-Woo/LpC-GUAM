...
function R = run_case(idx, val, onoff, cfg)
    ...
    hub = Config('brt_verify', struct('steps', cfg.M, 'target_vel', cfg.target_vel));
    ...
    nu = c.safety_filter.value_function.lon.axis_spec.nu;
    ...
    for k = 1:cfg.M
        ...
        if selPre.valid
            ...
            [lb, ub] = LivenessFilter.input_bounds(selPre.U0F, f.value_function.lon.axis_spec);
        end
...
function brt = make_brt_candidate(g, c, cfg, which)
    ...
    sf   = c.safety_filter;
    spec = sf.value_function.lon.axis_spec;
    ...
    [V, gradV, ok] = sf.value_function.lon.query(x, uh, wh);
...
function [V, ok, insideGrid] = query_fixed_candidate(g, c, brt)
    ...
    [V, ~, ok] = c.safety_filter.value_function.lon.query(xF, uhF, whF);

    spec = c.safety_filter.value_function.lon.axis_spec;
